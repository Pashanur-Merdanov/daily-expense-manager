...
$result = $conn->quotemeta(str)ery($sql);
$inctotal = 0;
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc())
    {
        ...
        echo "<tr><td> " . $exdate. "</td> <td> " . $row["income"]. " </td><td> " . $row["tvalue"]. "</td> <td> <a href='transactions.php?delincome=".$row['id']."' class='btn btn-danger btn-sm' name='remove' value='remove'><span class='glyphicon glyphicon-remove white' aria-hidden='true'></span></a></td></tr>";
        $inctotal+=$row["tvalue"];
